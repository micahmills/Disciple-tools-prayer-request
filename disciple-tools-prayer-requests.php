...
/**
 * Gets the instance of the `Disciple_Tools_Prayer_Requests` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
 */
function disciple_tools_prayer_requests() {
    $disciple_tools_prayer_requests_required_dt_theme_version = '1.19';
    ...
    if ( $is_theme_dt && version_compare( $version, $disciple_tools_prayer_requests_required_dt_theme_version, "<" ) ) {
        add_action( 'admin_notices', 'disciple_tools_prayer_requests_hook_admin_notice' );
    ...
    return Disciple_Tools_Prayer_Requests::instance();

}
add_action( 'after_setup_theme', 'disciple_tools_prayer_requests', 20 );
 ...
 * @access public
 */
class Disciple_Tools_Prayer_Requests {
    ...
    private function __construct() {
         ...
         */
        if ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-prayer-requests' ) !== false ) {
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-prayer-requests' );
    }
    ...
    public function __toString() {
        return 'disciple-tools-prayer-requests';
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( "disciple_tools_prayer_requests::" . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'Disciple_Tools_Prayer_Requests', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'Disciple_Tools_Prayer_Requests', 'deactivation' ] );


if ( ! function_exists( 'disciple_tools_prayer_requests_hook_admin_notice' ) ) {
    function disciple_tools_prayer_requests_hook_admin_notice() {
        global $disciple_tools_prayer_requests_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        $current_version = $wp_theme->version;
        $message = "'Disciple.Tools - Prayer Requests' plugin requires 'Disciple.Tools' theme to work. Please activate 'Disciple.Tools' theme or make sure it is latest version.";
        if ( $wp_theme->get_template() === "disciple-tools-theme" ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple.Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $disciple_tools_prayer_requests_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-prayer-requests', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-prayer-requests is-dismissible" data-notice="disciple-tools-prayer-requests">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-prayer-requests .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-prayer-requests',
 ...
 * @link https://github.com/DiscipleTools/disciple-tools-prayer-requests/wiki/Configuring-Remote-Updating-System
 *
...
//                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-prayer-requests/master/version-control.json',
//                __FILE__,
